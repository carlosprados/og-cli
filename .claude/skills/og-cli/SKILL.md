...
- **Forcing string type in `-w`.** `-w` infers the JSON type from the value: a value
  that is entirely an integer/float becomes a number, everything else (IPs, dotted
  versions, ISO timestamps) stays a string — so `-w "provision.device.identifier eq
  192.168.0.1-..."` works. For an **all-digit value that is actually a string** id,
  single-quote it to force string: `-w "provision.device.identifier eq '00123'"`.
...
`._current` is invalid. Works with `-w` (ISO is kept as a string) or raw `--filter`:

```bash
# Devices that COLLECTED ANYTHING after a moment → filter the always-collected identifier's at
og dev search -w "device.identifier._current.at gte 2026-06-21T18:00:00.000+02:00"
```

Project the timestamps too with `-s <stream>@at@date` (columns `<alias>_at` / `_date`).
...
| Each field wraps value as `_value._current.{value,at,date,source}` | **`at` = reception (platform ingest), `date` = measurement time**; project any sub-field with `-s <stream>@at@date@source` (or `--at` for `at` on all) |
...
| Artifacts are addressed by a generated `identifier`, not by `name` | true for timeseries and datasets (24-char hex), rules, connector functions and provision functions (UUID), workspaces and dashboards. Passing the human name fails; get the mapping from that family's `list` (or `search`). **Exceptions**: a datamodel's identifier *is* its name, and a device's is the `ogIdentifier` you already know |
| "Not found" has no single shape | timeseries answers HTTP 404 with `fields: identifier`, connector/provision functions 404 with `connectorFunctionId`/`provisionProcessorId`, datasets HTTP **400** `Element not found.`, rules HTTP **400** `No rule has been found with this id` — and workspaces, dashboards, datamodels and devices answer **HTTP 204 with an empty body**. Never key logic on the status code alone. From og 2.8.0 all three shapes come out as a clear error naming the family's listing command; before that, the 204 group printed an empty table or a JSON parse error and `og dev get <missing>` exited **0** |
