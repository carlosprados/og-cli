...
const (
	provisionProcessorsPath = "/north/v80/provisionProcessors/provision/organizations/%s"
	provisionProcessorPath  = "/north/v80/provisionProcessors/provision/organizations/%s/%s"
	provisionPlanPath       = "/north/v80/provisionProcessors/provision/organizations/%s/%s/plan?numberOfEntriesToProcess=%d"
	provisionBulkPath       = "/north/v80/provisionProcessors/provision/organizations/%s/%s/bulk"
	provisionBulkStatusPath = "/north/v80/provisionProcessors/provision/organizations/%s/bulk/%s"
	provisionBulkDetailPath = "/north/v80/provisionProcessors/provision/organizations/%s/bulk/%s/details"
)
...
func (c *Client) ListProvisionProcessors(org string) ([]json.RawMessage, error) {
...
func (c *Client) GetProvisionProcessor(org, id string) (json.RawMessage, error) {
...
func (c *Client) CreateProvisionProcessor(org string, body json.RawMessage) (json.RawMessage, error) {
	path := fmt.Sprintf(provisionProcessorsPath, org)
...
func (c *Client) UpdateProvisionProcessor(org, id string, body json.RawMessage) error {
	path := fmt.Sprintf(provisionProcessorPath, org, id)
...
func (c *Client) DeleteProvisionProcessor(org, id string) error {
	path := fmt.Sprintf(provisionProcessorPath, org, id)
...
func (c *Client) PlanProvisionBulk(org, id, filePath string, rows int) (json.RawMessage, error) {
	if rows <= 0 {
		rows = 1
	}
	...
	data, statusCode, _, err := c.PostMultipartFile(path, "file", filePath, "")
	if err != nil {
...
func (c *Client) RunProvisionBulk(org, id, filePath string) (string, error) {
	...
	data, statusCode, location, err := c.PostMultipartFile(path, "file", filePath, excelContentType(filePath))
	if err != nil {
...
func (c *Client) GetProvisionBulkStatus(org, bulkID string) (json.RawMessage, error) {
...
func (c *Client) GetProvisionBulkDetails(org, bulkID string) (data []byte, ready bool, err error) {
	...
	body, statusCode, err := c.GetWithAccept(path, excelContentType("x.xlsx"))
	if err != nil {
